...
function toAnswer(pct) {
  if (pct >= 0.05)  return 0; // 상승 (+5%+)
  if (pct > -0.05)  return 1; // 횡보 (-5%~+5%)
  if (pct > -0.15)  return 2; // 하락 (-5%~-15%)
  return 3;                   // 급락 (-15% 이하)
}

function answerLabel(ans) {
  return ['상승', '횡보', '하락', '급락'][ans];
}
...
// ---------- 설명 문장 ----------
// (ans × pattern) 기본 교훈 — 0=상승, 1=횡보, 2=하락, 3=급락
const LESSON_MAP = {
  0: { // 상승 (+5%+)
    uptrend:   '추세와 매크로가 같은 방향일 때 상승 에너지가 가장 강하다. 추세가 꺾일 때는 거래량 감소가 먼저 신호를 보낸다.',
    breakout:  '거래량 동반 돌파는 추세 추종 매매의 핵심 신호다. 돌파 이후 전 저항선이 지지로 바뀌는지 확인하는 것이 포지션 유지의 기준이 된다.',
    reversal:  '바닥에서의 강한 반등은 사이클 전환 초입을 알리는 신호다. 전 저점을 하회하지 않으면 매수세가 살아있다는 뜻이다.',
    downtrend: '하락 추세 중 역발상 반등은 드물지만, 매크로 개선 신호와 동반할 때 성공률이 높아진다.',
    sideways:  '박스권 돌파 후 상승은 억눌렸던 매수 에너지가 풀리는 패턴이다. 박스권이 길수록 돌파 후 지속력이 강하다.',
    doubletop: '쌍봉에서 강하게 반등했다면 고점 저항이 없는 새 국면이다. 전 고점 돌파는 다음 단계의 시작이다.',
    breakdown: '지지선 붕괴 후 강한 반등은 드물지만, 매크로가 개선될 때 V자 회복으로 이어지기도 한다.',
    parabolic: '포물선 상승은 모멘텀이 극도로 강할 때 나타난다. 방향이 맞으면 단기 수익이 크다.',
  },
  1: { // 횡보 (-5%~+5%)
    uptrend:   '상승 추세에서 숨 고르기가 나타났다. 눌림목을 추가 매수 기회로 볼지, 추세 종료 신호로 볼지가 핵심 판단이다.',
    breakout:  '돌파 시도가 무산되고 박스권으로 재진입했다. 거래량 없는 돌파는 페이크아웃 확률이 높다.',
    reversal:  '반등 후 재차 지지부진했다. 진짜 추세 전환과 데드캣 반등은 두 번째 저점 확인으로 구분할 수 있다.',
    downtrend: '하락이 일시 멈췄지만 추세 전환까지는 더 시간이 필요했다. 하락 추세에서 횡보는 이탈 전 최후의 경고일 수 있다.',
    sideways:  '박스권이 계속됐다. 방향성이 결정될 때까지 포지션 사이즈를 줄이는 것이 정답이다.',
    doubletop: '쌍봉 경고에도 불구하고 횡보로 에너지를 소화했다. 방향 결정이 지연되는 구간이다.',
    breakdown: '붕괴 이후 매도세와 저점 매수세가 균형을 이루며 횡보가 이어졌다.',
    parabolic: '급등 이후 속도 조절 구간으로 진입했다. 이격 해소 후 추세 방향이 재결정된다.',
  },
  2: { // 하락 (-5%~-15%)
    uptrend:   '올랐던 주식이 결국 꺾였다. 추세 종료 신호는 대개 거래량 이상과 고점 불경신으로 나타난다.',
    breakout:  '돌파처럼 보였지만 결국 실패하고 되돌렸다. 거래량 없는 돌파는 항상 의심해야 한다.',
    reversal:  '반등이 실패하며 하락이 나타났다. 첫 반등이 항상 바닥은 아니다. 전 저점 이탈 시 즉각 손절이 손실을 최소화한다.',
    downtrend: '하락 추세가 이어졌다. 약세장에서 저점 매수보다 추세 전환 신호 확인이 먼저다.',
    sideways:  '박스 하단을 이탈하며 하락이 나타났다. 오랜 횡보 후 이탈은 에너지가 아래로 풀리는 것이다.',
    doubletop: '쌍봉 패턴이 현실화됐다. 고점에서 호재에 반응이 없었다면 매수세 소진 신호다.',
    breakdown: '지지선 붕괴 이후 추가 하락이 이어졌다. 붕괴는 끝이 아닌 시작이다.',
    parabolic: '포물선 상승이 꺾이며 조정이 나타났다. 이격 해소는 결국 되돌림으로 이어진다.',
  },
  3: { // 급락 (-15% 이하)
    uptrend:   '오랜 상승 후 급락이 나타났다. 추세 종료 후의 낙폭은 예상보다 훨씬 크게 나타나는 경우가 많다.',
    breakout:  '돌파 실패가 급락으로 이어졌다. 거짓 돌파 후의 낙폭은 통상 돌파 시도 폭의 2배를 넘는다.',
    reversal:  '바닥인 줄 알고 진입했다가 급락이 나타났다. 하락 추세에서 매수는 추세 전환 확인 이후가 원칙이다.',
    downtrend: '하락 추세가 가속되며 급락이 나타났다. 약세장에서 "이쯤이면 바닥"이라는 감각은 항상 틀린다.',
    sideways:  '오랜 횡보 에너지가 아래로 한꺼번에 풀리며 급락이 나타났다. 지지 붕괴 시 즉각적 대응이 핵심이다.',
    doubletop: '쌍봉 패턴이 완성되며 급격한 하락이 이어졌다. 패턴 완성 후 목표가는 쌍봉 높이만큼 하락이다.',
    breakdown: '지지선 붕괴 후 패닉 매도가 급락을 만들었다. 붕괴는 끝이 아닌 시작이다.',
    parabolic: '포물선 끝에서 급격한 폭락이 나타났다. 어떤 추세도 영원히 지속될 수 없으며, 과열일수록 되돌림이 크다.',
  },
};
...
const EXPLANATION_CONTEXT = {
  ...
  '0_downtrend': '예상을 뒤집는 매수세가 유입되며 추세가 반전됐다.',
  '0_sideways':  '에너지 축적 이후 방향이 결정되자 빠르게 상승이 진행됐다.',
  '0_doubletop': '고점 저항을 돌파하며 쌍봉 패턴이 무력화됐다.',
  '0_breakdown': '붕괴 직후 강한 매수세가 유입되며 V자 반등이 나타났다.',
  '0_parabolic': '포물선 가속이 유지되며 추가 상승이 이어졌다.',
  '1_breakout':  '거래량 없는 돌파는 페이크아웃으로 끝나며 박스권으로 회귀했다.',
  '1_uptrend':   '단기 모멘텀이 소진되며 관망세가 늘어 방향성이 약해졌다.',
  '1_reversal':  '반등 시도가 이어졌지만 매도 매물이 쌓여 방향을 결정짓지 못했다.',
  '1_downtrend': '하락 속도가 줄었지만 매수 주체가 충분하지 않아 추세 전환까지는 이르지 못했다.',
  '1_sideways':  '특별한 촉매 없이 박스권이 그대로 유지됐다.',
  '1_doubletop': '고점 경계에서 매수·매도 균형으로 방향성이 없었다.',
  '1_breakdown': '붕괴 이후 반등과 재하락이 반복되며 방향을 정하지 못했다.',
  '1_parabolic': '급등 후 이격 해소 과정에서 등락이 반복됐다.',
  '2_breakout':  '수급이 뒷받침되지 않은 돌파였고, 결국 되돌림 하락이 나타났다.',
  '2_uptrend':   '상승 피로가 누적된 상태에서 매도 세력이 우위를 점하며 추세가 꺾였다.',
  '2_reversal':  '반등이 실패하며 하락이 나타났다. 첫 반등이 바닥을 확정하진 않는다.',
  '2_downtrend': '매도 압력이 지속되며 하락이 이어졌다.',
  '2_sideways':  '지지선이 이탈되며 하락 에너지가 쏟아졌다.',
  '2_doubletop': '쌍봉 패턴이 현실화되며 하락이 나타났다.',
  '2_breakdown': '붕괴 후 추가 매도세가 유입되며 하락이 지속됐다.',
  '2_parabolic': '포물선 상승이 꺾이며 되돌림 하락이 나타났다.',
  '3_breakout':  '수급과 매크로가 모두 역방향이었고, 결국 급격한 되돌림으로 이어졌다.',
  '3_uptrend':   '고점 과열이 해소되며 빠른 속도로 추세가 역전됐다. 낙폭은 오른 폭의 절반을 쉽게 넘겼다.',
  '3_reversal':  '반등 실패 후 패닉 매도가 가세하며 급락이 발생했다. 잘못된 저점 매수의 전형이다.',
  '3_downtrend': '하락 추세가 가속화되며 급락으로 이어졌다. 악재가 겹칠수록 낙폭은 예상을 넘는다.',
  '3_sideways':  '긴 횡보 에너지가 아래로 한꺼번에 풀리며 급락이 나타났다.',
  '3_doubletop': '쌍봉 패턴이 완성되며 네크라인 이탈 후 급락이 가속됐다.',
  '3_breakdown': '붕괴 후 패닉 매도가 가세하며 급락이 이어졌다.',
  '3_parabolic': '포물선 끝에서 과열 해소 급락이 나타났다. 상승 속도가 빠를수록 되돌림도 가파르다.',
};
...
function makeExplanation(ans, pctStr, months, pattern) {
  ...
  if (ans === 0) { // 상승
    verb = {
      ...
      uptrend:   '추세를 이어가며',
      sideways:  '박스권을 돌파하며',
      doubletop: '고점 저항을 돌파하며',
      breakdown: '급반등하며',
      parabolic: '상승 모멘텀을 유지하며',
    }[pattern] || '이후';
  } else if (ans === 1) { // 횡보
    verb = {
      ...
      uptrend:   '상승 피로 누적 후',
      sideways:  '방향성 없이',
      doubletop: '고점 부근에서 균형을 이루며',
      breakdown: '붕괴 이후 반등·재하락을 반복하며',
      parabolic: '급등 이후 이격 해소로',
    }[pattern] || '이후';
  } else if (ans === 2) { // 하락
    verb = {
      reversal:  '반등 실패 후',
      ...
      doubletop: '쌍봉 완성 후',
      breakdown: '붕괴 이후',
      parabolic: '과열 해소로 되돌리며',
    }[pattern] || '이후';
  } else { // 급락
    verb = {
      reversal:  '반등 실패 후 패닉 매도가 가세하며',
      downtrend: '하락이 가속되며',
      breakout:  '돌파 실패 후 급격히 되밀리며',
      uptrend:   '추세가 역전되며',
      sideways:  '지지선이 붕괴되며',
      doubletop: '쌍봉 패턴이 완성되며',
      breakdown: '붕괴 이후 패닉 매도로',
      parabolic: '포물선 끝에서',
    }[pattern] || '이후';
  }
...
// ---------- 분포 목표 (GEN 전체) ----------
// HC 재매핑 후: 상승~29, 횡보15, 하락~5, 급락~2
// GEN 목표: 상승64, 횡보65, 하락25, 급락10 = 164개
const DIST_TARGET = { 0: 64, 1: 65, 2: 25, 3: 10 };
...
for (const ans of [0, 1, 2, 3]) {
  ...
  console.log(`  답=${ans}(${['상승','횡보','하락','급락'][ans]}): 후보 ${pool.length}개 → ${take}개 선택`);
}
...
// ---------- 패턴별 choices (0=상승, 1=횡보, 2=하락, 3=급락) ----------
