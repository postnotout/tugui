...
const TEMPLATES = {
  uptrend: [
    (n, m) => `${n}이(가) 꾸준히 우상향하며 매수세가 계속 몰리고 있어요. ${m}개월 뒤 주가는 어떻게 됐을까요?`,
    (n, m) => `${n} 주가가 이평선 위에서 강한 상승 흐름을 이어가는 중입니다. ${m}개월 후 결과는?`,
    (n, m) => `${n}이(가) 정배열 상태에서 거래량까지 늘고 있어요. 이 추세, ${m}개월 뒤에도 이어졌을까요?`,
    (n, m) => `${n}이 연속으로 신고가를 갱신하며 상승 중이에요. ${m}개월 뒤 어떻게 됐을까요?`,
    (n, m) => `${n} 주가가 MA20 위에서 완만하게 우상향 중입니다. ${m}개월 뒤 결과는?`,
  ],
  downtrend: [
    (n, m) => `${n}이(가) 연속으로 신저가를 갱신하며 하락 중이에요. ${m}개월 뒤 주가는?`,
    (n, m) => `${n} 주가가 이평선 아래에서 매도세가 이어지고 있습니다. ${m}개월 후 결과는?`,
    (n, m) => `${n}이 계속 빠지고 있어요. 이 하락세, ${m}개월 뒤에 어떻게 됐을까요?`,
    (n, m) => `${n}이(가) 역배열 상태에서 반등 없이 흘러내리고 있습니다. ${m}개월 후는?`,
    (n, m) => `${n} 주가가 고점 대비 많이 빠진 상태예요. 지금부터 ${m}개월, 어떻게 됐을까요?`,
  ],
  breakout: [
    (n, m) => `${n}이(가) 오랫동안 막혀 있던 저항선을 뚫었어요. 이번엔 진짜일까요? ${m}개월 후?`,
    (n, m) => `${n}이 박스권 상단을 돌파하며 거래량이 터졌습니다. ${m}개월 뒤 어떻게 됐을까요?`,
    (n, m) => `${n} 주가가 52주 신고가를 경신하며 돌파에 성공했어요. ${m}개월 뒤 결과는?`,
    (n, m) => `${n}이(가) 고점 저항을 돌파하는 강한 양봉이 나왔습니다. ${m}개월 후?`,
    (n, m) => `${n}이 중요 저항선을 상향 돌파했어요. 추세 전환이 시작된 걸까요? ${m}개월 뒤는?`,
  ],
  reversal: [
    (n, m) => `${n}이(가) 크게 빠진 후 바닥에서 반등 신호가 나왔어요. ${m}개월 뒤 어떻게 됐을까요?`,
    (n, m) => `${n} 주가가 급락 이후 저점에서 강한 양봉이 나오기 시작했습니다. ${m}개월 후?`,
    (n, m) => `${n}이 하락을 멈추고 바닥을 다지는 것 같아요. ${m}개월 뒤 결과는?`,
    (n, m) => `${n}이(가) 과매도 구간에서 거래량을 동반한 반등 시도를 하고 있어요. ${m}개월 후?`,
    (n, m) => `${n} 주가가 긴 하락 후 지지선에서 힘을 받는 모습입니다. ${m}개월 뒤 어떻게 됐을까요?`,
  ],
  sideways: [
    (n, m) => `${n}이(가) 박스권에서 방향을 잡지 못하고 오르내리고 있어요. ${m}개월 뒤 어디로 갔을까요?`,
    (n, m) => `${n} 주가가 일정 범위 안에서만 움직이고 있습니다. ${m}개월 뒤 탈출에 성공했을까요?`,
    (n, m) => `${n}이 횡보 구간에서 돌파 시도를 이어가고 있어요. ${m}개월 뒤 결과는?`,
    (n, m) => `${n} 주가가 추세 없이 박스권을 유지 중이에요. ${m}개월 뒤 어떻게 됐을까요?`,
    (n, m) => `${n}이(가) 눌림목 구간에서 쉬어가고 있습니다. ${m}개월 후 방향은?`,
  ],
};
